...
class SpiffyStoresApp::ScripttagsManagerTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    @manager = SpiffyStoresApp::ScripttagsManager.new(@scripttags, 'example-app.com')
  end

  test "#create_scripttags makes calls to create scripttags" do
    SpiffyStoresAPI::ScriptTag.stubs(all: [])
  ...
  test "#create_scripttags when creating a dynamic src, does not overwrite the src with its result" do
    ...
    SpiffyStoresAPI::ScriptTag.expects(:create).returns(stub_scripttag).times(3)
  ...
  test "#create_scripttags when creating a scripttag fails, raises an error" do
    ...
    SpiffyStoresAPI::ScriptTag.stubs(create: scripttag)

    e = assert_raise SpiffyStoresApp::ScripttagsManager::CreationFailed do
      @manager.create_scripttags
    end
  ...
  test "#create_scripttags when a script src raises an exception, it's propagated" do
    SpiffyStoresAPI::ScriptTag.stubs(:all).returns(all_mock_scripttags[0..1])
  ...
  test "#destroy_scripttags makes calls to destroy scripttags" do
    SpiffyStoresAPI::ScriptTag.stubs(:all).returns(Array.wrap(all_mock_scripttags.first))
    SpiffyStoresAPI::ScriptTag.expects(:delete).with(all_mock_scripttags.first.id)

    @manager.destroy_scripttags
  end

  test "#destroy_scripttags makes calls to destroy scripttags with a dynamic src" do
    SpiffyStoresAPI::ScriptTag.stubs(:all).returns(Array.wrap(all_mock_scripttags.last))
    SpiffyStoresAPI::ScriptTag.expects(:delete).with(all_mock_scripttags.last.id)

    @manager.destroy_scripttags
  end

  test "#destroy_scripttags when deleting a dynamic src, does not overwrite the src with its result" do
    SpiffyStoresAPI::ScriptTag.stubs(all: Array.wrap(all_mock_scripttags.last))
    SpiffyStoresAPI::ScriptTag.expects(:delete).with(all_mock_scripttags.last.id)
  ...
  test "#destroy_scripttags does not destroy scripttags that do not have a matching address" do
    SpiffyStoresAPI::ScriptTag.stubs(:all).returns([stub(src: 'http://something-or-the-other.com/badscript.js', id: 7214109)])
    SpiffyStoresAPI::ScriptTag.expects(:delete).never
  ...
  test ".queue enqueues a ScripttagsManagerJob" do
    ...
    assert_enqueued_with(job: SpiffyStoresApp::ScripttagsManagerJob, args: [args]) do
      SpiffyStoresApp::ScripttagsManager.queue(args[:shop_domain], args[:shop_token], @scripttags[-1,1])
    end
  end
  ...
  def expect_scripttag_creation(event, src)
    stub_scripttag = stub(persisted?: true)
    SpiffyStoresAPI::ScriptTag.expects(:create).with(event: event, src: src, format: 'json').returns(stub_scripttag)
  end
