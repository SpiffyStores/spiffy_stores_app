...
class SpiffyStoresApp::WebhooksManagerTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    @manager = SpiffyStoresApp::WebhooksManager.new(@webhooks)
  end

  test "#create_webhooks makes calls to create webhooks" do
    SpiffyStoresAPI::Webhook.stubs(all: [])
  ...
  test "#create_webhooks when creating a webhook fails, raises an error" do
    ...
    SpiffyStoresAPI::Webhook.stubs(create: webhook)

    e = assert_raise SpiffyStoresApp::WebhooksManager::CreationFailed do
      @manager.create_webhooks
    end
  ...
  test "#create_webhooks doesn't create webhooks that are already created" do
    SpiffyStoresAPI::Webhook.stubs(all: all_mock_webhooks)
  ...
  test "#destroy_webhooks doesnt freak out if there are no webhooks" do
    SpiffyStoresAPI::Webhook.stubs(:all).returns(nil)

    @manager.destroy_webhooks
  end

  test "#destroy_webhooks makes calls to destroy webhooks" do
    SpiffyStoresAPI::Webhook.stubs(:all).returns(Array.wrap(all_mock_webhooks.first))
    SpiffyStoresAPI::Webhook.expects(:delete).with(all_mock_webhooks.first.id)

    @manager.destroy_webhooks
  end

  test "#destroy_webhooks does not destroy webhooks that do not have a matching address" do
    SpiffyStoresAPI::Webhook.stubs(:all).returns([stub(address: 'http://something-or-the-other.com/webhooks/product_update', id: 7214109)])
    SpiffyStoresAPI::Webhook.expects(:delete).never
  ...
  def expect_webhook_creation(topic, address)
    ...
    SpiffyStoresAPI::Webhook.expects(:create).with(topic: topic, address: address, format: 'json').returns(stub_webhook)
  end
